...
page format used by the `explain-diff` skill (see ./SKILL.md).

Recipe from https://gist.github.com/geoffreylitt/a29df1b5f9865506e8952488eac3d524;
renderer + JSON-spec split from
https://gist.github.com/ankitg12/8e808d387799de4e9839bc393f8e6405. Local changes
for CareerVault: the default output path (docs/explanations/) and --fragment.
...
Usage:
    python3 render.py spec.json [-o output.html] [--fragment]

If -o is omitted, writes to docs/explanations/YYYY-MM-DD-<slug>.html relative to
the repo root, where <slug> comes from the spec's "slug" field. --fragment emits
the page body without the <!doctype>/<head>/<body> wrapper, which is the shape
the Artifact tool expects.
...
def render(spec: dict, *, fragment: bool = False) -> str:
    title = spec["title"]
...
"""
    if fragment:
        # Artifact publishing wraps the file in its own <!doctype>/<head>/<body>, and a <title>
        # tag names the page in the browser tab + gallery.
...
def main():
    ...
    ap.add_argument(
        "--fragment",
        action="store_true",
        help="emit the body only (no doctype/head/body) for publishing via the Artifact tool",
    )
    args = ap.parse_args()

    spec = json.loads(args.spec.read_text(encoding="utf-8"))
    out_html = render(spec, fragment=args.fragment)
    ...
    if args.output:
        ...
        # Repo-relative: these explainers are committed alongside the code they explain.
        out_path = Path(__file__).resolve().parents[3] / "docs" / "explanations" / f"{date_prefix}-{slug}.html"
