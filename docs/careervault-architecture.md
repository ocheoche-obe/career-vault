...
|---|---|---|---|---|
| **`chat_lambda`** | `Query` (`CONVO#*` **and `ENTRY#*`, read-only** — slice 7), `PutItem` (`CONVO#*` only) | `InvokeModel` on `<haiku>`, `<titan-embed>` | — | — |
...
>
> - **What chat can now do:** `Query` its own partition for `ENTRY#` items, and `InvokeModel` on Titan to embed a retrieval query. Both are additive reads; nothing about the write path changed.
