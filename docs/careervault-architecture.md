...
| Résumé (history record) | `USER#<user_id>` | `RESUME#<run_id>` | — |
| Résumé run trace | `USER#<user_id>` | `RESUMERUN#<run_id>` | 30 days |
| Check-in audit log | `USER#<user_id>` | `CHECKINLOG#<run_id>` | — |

> **Doc correction (v1.1 slice 3).** This table listed only the original four entities and had drifted from the deployed table for two slices: `RESUMERUN#` shipped in slice 6a (ADR-037) and `CHECKINLOG#` in slice 8, neither of which was added here. Both are recorded above along with `RESUME#` (ADR-046) rather than adding one row and leaving the older omissions in place.
>
> **The TTL column is new, and it is the point of ADR-046.** DynamoDB's TTL only ever deletes items carrying the `expires_at` attribute, so "no TTL" is not a configuration — it is an item that simply omits the field. `RESUME#` and `RESUMERUN#` are the same run seen twice: the résumé the user built (durable) and the agent exhaust that produced it (30 days). ⚠️ The two prefixes differ only at their 7th character; `begins_with("RESUME#")` excludes `RESUMERUN#` **only because the `#` is part of the prefix**.
...
| AP-13 List résumé history (ADR-046) | `Query(PK=USER#u, begins_with(SK,"RESUME#"), ScanIndexForward=False, ProjectionExpression=…)` |

**AP-13 needs no GSI, and ADR-028 holds.** `run_id` is a ULID, so SK order *is* time order and `ScanIndexForward=False` returns newest-first directly. The projection is not an optimisation detail but a correctness-of-scale one: the item stores the full target text and the structured résumé, neither of which a list of cards renders (cf. B-013).
...
>
> **Addition (v1.1 slice 3 — ADR-046).** A fourth route, **`GET /resumes`**, returns the user's
> résumé history newest-first (AP-13). On success the worker now writes a second, *durable* item —
> `RESUME#<run_id>`, carrying no `expires_at` — alongside the 30-day `RESUMERUN#` trace; a failed run
> writes only the trace, so "past résumés" means résumés that exist. The two writes are ordered
> trace-then-record and are deliberately **not** a transaction (ADR-046 §3).
>
> **Addition (v1.1 slice 3 — ADR-046 amendment 2).** A fifth route, **`DELETE /resumes/{run_id}`**,
> removes a résumé behind ADR-027's UI confirm: **S3 artifacts first**, then the `RESUME#` record,
> then any surviving `RESUMERUN#` trace. The ordering is the inverse of the write path and chosen for
> the same reason — it picks which divergence a mid-way crash produces. Deleting the objects first
> leaves a row whose View fails *visibly* and which the user can delete again (the operation is
> idempotent); deleting the record first would leave S3 objects that nothing references and — since
> this slice removed the lifecycle rule — nothing will ever expire (**B-039**). This route is why
> `resume_agent` now holds `dynamodb:DeleteItem` and `s3:DeleteObject` on `resumes/*`; those are the
> only IAM additions in the slice.
...
>
> **Superseded (v1.1 slice 3, ADR-046).** Generated artifacts no longer expire at all: the `resumes/` lifecycle rule is removed and only the DynamoDB trace still TTLs at 30 days. The flat 30 days above was never a judgment about how long a résumé is worth keeping — it was chosen so a trace item could not outlive the objects it pointed at, and the durable `RESUME#` record retired that coupling. `GET /resumes/{run_id}` therefore falls back to the record when the trace has aged out, so a résumé older than 30 days still opens.
...
| 2.2     | 2026-08-08 | v1.1 slice 3 (résumés + résumé history) — **ADR-046**, plus a §2.4 table that had drifted for two slices. **§2.4:** the PK/SK table listed only the original four entities; `RESUMERUN#` (slice 6a) and `CHECKINLOG#` (slice 8) were never added. All three are now recorded together with the new `RESUME#`, rather than adding one row and leaving the older omissions — and the table gains a **TTL column**, because that column is the whole of ADR-046: DynamoDB's TTL only deletes items carrying `expires_at`, so "durable" is an *omitted attribute*, not a configuration. `RESUME#` and `RESUMERUN#` are one run seen twice — the résumé the user built, and the agent exhaust that produced it. ⚠️ The prefixes differ only at the 7th character; `begins_with("RESUME#")` excludes `RESUMERUN#` **only because the `#` is part of the prefix**. **§2.5:** new **AP-13** (list résumé history) — a projected `Query` with `ScanIndexForward=False`, needing **no GSI** (ULID sort keys make SK order time order), so ADR-028 holds; the projection is correctness-of-scale rather than tuning, since the item stores the full target text and structured résumé that a card list never renders (cf. B-013). **§3.2.1:** a fourth route, `GET /resumes`. **§3.2.2 Phase 6 retention superseded:** the flat 30-day `resumes/` lifecycle rule from the ADR-015 amendment is **removed** — that number was never a judgment about how long a résumé is worth keeping, only a guard against a trace item outliving the objects it pointed at, and the durable record retires that coupling. Consequence handled rather than discovered: with artifacts now outliving the trace, `GET /resumes/{run_id}` **falls back to the `RESUME#` record**, so the list cannot show a row whose View 404s. Also this slice: **B-022** (structured document returned on the poll, making plain-text bullets copyable — the last third of FR-5.3) and **B-007** (elapsed time carried into the completed run). **Added mid-slice (2026-08-09), ADR-046 amendment 2:** a fifth route **`DELETE /resumes/{run_id}`** — S3 artifacts, then record, then trace, behind ADR-027's confirm. Removing the lifecycle rule is what *created* the need: nothing clears a résumé automatically any more. It brings the slice's only IAM additions, `dynamodb:DeleteItem` and `s3:DeleteObject` on `resumes/*`. No architectural decisions reversed; ADR-015's delivery half stands unchanged. |
