...
**Version:** 1.2
**Status:** **Complete** — architecture phase finalized (1.1–1.2: implementation-driven clarifications)
...
The current system architecture diagram is maintained as code in `render_architecture.py` (using the `diagrams` / mingrammer library) and rendered to `careervault_architecture.png` and `careervault_architecture.pdf`. Regenerate by running `python render_architecture.py` after architecture changes.
...
**User & edge.** The user's browser is the only client. Static assets (the React app bundle) come from a private S3 bucket fronted by CloudFront via Origin Access Control. The bucket has no public access — only CloudFront's SigV4-signed requests can reach it. ACM provisions the TLS cert; Route 53 maps the custom domain to the CloudFront distribution. This is the standard pattern locked in by ADR-019.
...
| `max_iterations` reached (15) | Phase 2 | Exit retrieval with whatever was gathered |
| `max_revisions` reached (2) | Phase 5 | Finalize with current draft, log "revisions exhausted" |
| `token_budget_ceiling` exceeded (500K cumulative) | All Bedrock calls | Graceful failure; persist partial trace; 500 to user |
...
The token-budget ceiling deserves a brief note: every Bedrock response includes `usage.inputTokens` and `usage.outputTokens`. A running cumulative sum gates the loop. At Sonnet's pricing (~$3/M input, ~$15/M output), 500K cumulative tokens is roughly $3–4 — well above expected per-run cost (~$0.10–0.30) but well below "something is catastrophically wrong" territory.
...
> **Multi-tenant evolution.** Once we have many users with individual time-zone preferences, the cleaner pattern is per-user EventBridge Scheduler entries, created at signup and updated when a user changes their cadence or preferred time. EventBridge Scheduler supports millions of schedules per account, so this scales fine. The data model (PROFILE has `checkin_cadence` and `checkin_time_local` attributes already) supports either path — only the provisioning logic differs.

**Due-user query.**
The Lambda queries PROFILE items where `next_checkin_at <= now`. For MVP this is a single record; for multi-tenant this would benefit from a GSI keyed by `next_checkin_at` (deferred — see Section 2 GSI strategy). The result is the list of users to process in this invocation.
...
|---|---|---|---|---|
| **`chat_lambda`** | `Query`, `PutItem` | `InvokeModel` on `<haiku>` | — | — |
| **`career_crud`** | `GetItem`, `Query`, `PutItem`, `UpdateItem`, `DeleteItem` | `InvokeModel` on `<titan-embed>` | — | — |
...
def put_conversation_message(user_id: str, session_id: str, message: dict) -> None:
    """Write a CONVO message. Conditional write enforces the CONVO# invariant."""
    ...
    table.put_item(
        ...
        # Defense in depth: even if upstream code were tricked into constructing
        # a different SK, the conditional write enforces the CONVO# invariant.
        ConditionExpression="attribute_not_exists(SK) AND begins_with(SK, :prefix)",
        ExpressionAttributeValues={":prefix": "CONVO#"},
    )
```

The `begins_with` clause is the belt-and-suspenders piece — if any upstream bug constructed an SK that didn't start with `CONVO#`, the conditional check fails and the write is rejected before it lands. Same pattern in `career_crud` for `ENTRY#*`, `resume_agent` for `RESUMERUN#*`, `checkin_lambda` for `CHECKINLOG#*`. The helper functions live in the shared Lambda layer so the pattern is enforced uniformly.

This is a small defensive measure but worth committing to — the alternative invariant (*"the application code constructs SKs correctly, trust me"*) is exactly the kind of property that holds until a refactor accidentally breaks it without anyone noticing.
...
| IAM (function-specific) | `dynamodb:UpdateItem` on the table, with PROFILE-scope enforced in application code per 4.2.4 |
...
2. Resolve the recipient to a `user_id` via the PROFILE's `email` attribute (single Query against the table; multi-tenant scale would warrant a GSI on `email`, which is a v2 problem).
...
**Bulk-upload nuance.** `resume_upload_parser` can extract many entries from one upload (often 5–20, occasionally more). N sequential Titan calls would compound latency linearly. Titan v2 supports **multiple inputs per request**, so the bulk-parse path batches them — typically 25 entries per Titan call — keeping the total embedding latency to one or two API calls regardless of upload size. The shared Lambda layer's embedding helper exposes both single and batch variants so each caller picks the right one.
...
2. A one-off backfill Lambda is invoked manually. It paginates through entries via the AP-10 read, filters those where `embedding_model != <new model>`, re-embeds in batches via Titan's batch input, and UpdateItems the new vectors.
...
| `resume_upload_parser` | 5 min (300s) | Bulk parsing + batched embedding |
...
**Frontend deploy** is a separate flow since CloudFront serves the React bundle from S3 directly per ADR-019:

```bash
cd frontend
npm run build                                           # Produces dist/
aws s3 sync dist/ s3://careervault-web-prod-<account>/ --delete
aws cloudfront create-invalidation \
    --distribution-id <distribution-id> \
    --paths "/*"
```

The CloudFront invalidation matters: without it, the CDN may continue serving stale cached versions for up to the cache TTL after the S3 sync. The distribution ID comes from the `CloudFrontDistributionId` output in 5.4.

A small `Makefile` at the repo root wraps both steps into a single `make deploy` command (and `make deploy-prod`), keeping the muscle memory simple.
