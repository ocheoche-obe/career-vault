...
| 2.1 | Ingestion ≤ 5s | ✅ **Met, measured** | **Measured 2026-08-21** (v1.1 slice 5, deployed dev, ADR-047 harness, `--bedrock` tier). "User submits text → confirmation shown" is the `POST /chat` parse turn: **4,307 ms cold** (1,305 ms init + 2,278 ms handler, of which ~2.3 s is the Haiku call) and **1,638 ms warm**. The confirm write that follows adds **239 ms** warm, including its inline Titan embed (ADR-024). Passes cold and warm — but thinly cold, at 4.3 s against a 5 s budget, so this would not survive the parse turn getting much slower. **Closes the NFR-2.1 half of B-023, which is now fully closed.** |
