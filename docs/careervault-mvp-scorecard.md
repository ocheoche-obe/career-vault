...
| 2.1 | Ingestion ≤ 5s | ✅ **Met, measured** | **Measured 2026-08-21** (v1.1 slice 5, deployed dev, ADR-047 harness, `--bedrock` tier). The requirement's own parenthetical — "user submits text → confirmation shown" — is the `POST /chat` parse turn: **4,392 ms cold** (1,208 ms init + 2,438 ms handler, most of it the Haiku call) and **~1,710 ms warm**. ✅ against the 5,000 ms budget, but thinly cold; it would not survive the parse turn getting much slower. ⚠️ **A stricter reading fails.** If "ingestion complete" means the entry is *saved*, the confirm write follows at **2,798 ms cold** (1,337 ms init + 888 ms handler, including the inline Titan embed per ADR-024) / **346 ms warm** — so a fully cold submit-then-confirm is **~7.2 s**, over budget. Scored against the wording as written. **Closes the NFR-2.1 half of B-023, which is now fully closed.** |
| 2.2 | Résumé generation ≤ 4 min, async | ✅ | Corrected in v0.6 from an unmeetable 30s. **Improved in v1.1 slice 5: 81.8 s → 48.4 s (−41%) and $0.1233 → $0.0785 (−36%)** on the 2-entry fixture (ADR-048 + the Haiku critique swap). The attribution is the interesting part — prompt caching bought the *cost* and none of the latency; the critique model swap bought essentially all of it. Real-corpus figure (~176 s at 13 entries) has not been re-measured. |
| 2.3 | Dashboard load ≤ 2s | ⚠️ **Met warm, failed cold** | **Measured 2026-08-07** (v1.1 slice 1, 13-entry corpus, deployed dev API) — see [pre-redesign audit §D1](design/v1.1-redesign/pre-redesign-audit.md). Cold **3686 ms**; warm 721–1133 ms (mean ~940 ms). Nearly all of it is `GET /entries` (3639 ms cold, ~460 ms warm), not rendering. **The cold path is the typical path for this app**: FR-4's check-in email brings the user back weekly-to-monthly, so the Lambda is reliably cold when they arrive — a warm average would be the wrong number to report. **Re-measured 2026-08-21** (v1.1 slice 5) with per-term decomposition, and the *attribution above was wrong*: cold **2,620 ms** = 1,053 ms container init + 778 ms handler + wire, against **300 ms warm**. The corpus term is only ~188 ms (~14.5 ms/entry, from a 6 ms empty-corpus handler), so **B-013 is ~8% of a cold load, not the cause of the miss** — cold start is. Projecting embeddings out would buy ~190 ms against a ~2,300 ms problem. Tracked as **B-047**; deliberately not fixed in slice 5, whose scope was résumé speed. Closes the NFR-2.3 half of **B-023**. |
...
| 6.2 | Usable on desktop **and mobile web** | ⚠️ **Overflow fixed; full pass pending** | **Measured, then fixed, 2026-08-07** (v1.1 slice 1) — see [pre-redesign audit §D2](design/v1.1-redesign/pre-redesign-audit.md). **Before:** all five views overflowed horizontally by **82px** at 360px — cause entirely shell-local (`.app-header` at `width: min(720px, 100%)` carrying a 241px email string that could not compress, and a `.view-nav` that neither wrapped nor scrolled). **After the slice-1 shell rebuild: 0px overflow on all six views at 375px**, verified in-browser. Caveated rather than ✅ deliberately: overflow is one measurable property of "usable on mobile", and the views behind the shell still carried their pre-redesign internal layouts. **Updated after v1.1 slice 2** (PR #48): Log, Timeline, Import and Details are now redesigned responsively, leaving **Résumés as the only view on a pre-redesign layout** — it is deferred to slice 3 because it is blocked on **B-028**, and until then it renders inside the temporary `.legacy-view` wrapper. *(The earlier note here said to re-score to ✅ once slice 2 landed; that was written before Résumés was split out, and following it would have marked the requirement met while one view was still unconverted.)* **Re-score to ✅ when slice 3 lands and `.legacy-view` is deleted (B-036)** — and re-measure rather than assume, since the original 82px failure was invisible until someone drove a mobile viewport. → **B-001** |
