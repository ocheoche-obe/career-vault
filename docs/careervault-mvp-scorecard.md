...
| 6.2 | Usable on desktop **and mobile web** | ❌ **Failed** | **Measured 2026-08-07** (v1.1 slice 1) — see [pre-redesign audit §D2](design/v1.1-redesign/pre-redesign-audit.md). At a 360px viewport **all five views overflow horizontally by 82px**. Cause is entirely the shell, not the views: `.app-header` is `width: min(720px, 100%)` carrying a 241px email string that cannot compress, and `.view-nav` neither wraps nor scrolls. Was scored ❓Unverified because it had never been driven at a mobile viewport; it now has a number, and the number is a failure. Being shell-local is the good news — the v1.1 slice-1 header rebuild clears all five views at once. → **B-001** |
